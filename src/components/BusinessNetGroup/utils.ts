...
export function getCleanNetGroupIds(businessGroupIds?: string) {
...
export function getDefaultBusinessGroupKey() {
  ...
  const defaultBusinessGroupKey =
    ...
    window.localStorage.getItem('netGroupKey') ||
    undefined;
...
export function getDefaultNetGroup(busiGroups) {
  ...
  if (!isValid) {
    ...
    ids = getCleanNetGroupIds(defaultBusinessGroupKey);
    idsArr = _.map(_.compact(_.split(ids, ',')), _.toNumber);
  }
  if (defaultBusinessGroupKey) {
    window.localStorage.setItem('netGroupKey', defaultBusinessGroupKey);
    return {
...
export function getVaildNetGroup(
  busiGroups,
  businessGroupKey: {
    key: string;
    ids: string;
  ...
  let ids = getCleanNetGroupIds(businessGroupKey.key);
  let idsArr = _.map(_.compact(_.split(ids, ',')), _.toNumber);
  const isValid = !_.isEmpty(idsArr)
    ? _.every(idsArr, (id) => {
        if (!_.find(busiGroups, { id })) {
    ...
    : false;
  if (!isValid) {
    return getDefaultNetGroup(busiGroups);
  }
