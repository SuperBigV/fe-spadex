...
import { changeBusinessTeam, getBusinessTeamInfo } from '@/services/manage';
const { Option } = Select;
const TeamForm = React.forwardRef<ReactNode, ModelProps>((props, ref) => {
  ...
  useEffect(() => {
    if (modelId) {
      getTeamInfoDetail(modelId);
    } else {
      setLoading(false);
    }
  }, []);
  const getTeamInfoDetail = async (id: number) => {
    const data = await getBusinessTeamInfo(id);
    setInitialValues({
      id: data.id,
      name: data.name,
    });
