...
import CreateNetGroup from '@/pages/netTargets/components/CreateTree';
import { getBusinessTeamList } from '@/services/manage';
import { listToTree, getCollapsedKeys, getCleanNetGroupIds, getDefaultBusinessGroupKey, getCleanBusinessGroupIds, getDefaultNetGroup, getVaildNetGroup } from './utils';
import BusinessGroupSelect from './BusinessGroupSelect';
import BusinessGroupSelectWithAll from './BusinessGroupSelectWithAll';
import { getNetGroups } from './services';
import './style.less';
...
export default function index(props: IProps) {
  ...
  const { netGroup, netGroupOnChange } = useContext(CommonStateContext);
  const location = useLocation();
  const query = queryString.parse(location.search);
  const history = useHistory();
  const { title = t('网络分组'), renderHeadExtra, onSelect, pageKey, showSelected = true } = props;
  ...
  const { netGroups, siteInfo, setNetGroups } = useContext(CommonStateContext);
  ...
  const reloadData = () => {
    getBusinessTeamList({ limit: 5000, typ: 'net' }).then((res = []) => {
      setNetGroups(res);
      const filteredData = filterData(searchValue, res.dat);
  ...
  useEffect(() => {
    ...
    if (savedSearchValue) {
      ...
      data = filterData(savedSearchValue, netGroups);
    }
        ...
        <div className='n9e-biz-group-container-group group-shrink'>
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              const filteredData = filterData(value, netGroups);
              setBusinessGroupTreeData(listToTree(filteredData, siteInfo?.businessGroupSeparator));
            }}
          ...
          {siteInfo?.businessGroupDisplayMode == 'list' ? (
            ...
            <div className='radio-list'>
              {!_.isEmpty(businessGroupTreeData) && (
                <Tree
                  defaultExpandedKeys={getCollapsedKeys(businessGroupTreeData, getLocaleExpandedKeys(), netGroup.key)}
                  selectedKeys={showSelected && netGroup.key ? [netGroup.key] : undefined}
                  onSelect={(_selectedKeys, e) => {
                    ...
                    history.push({
                      ...
                      search: queryString.stringify({
                        ...query,
                        ids: getCleanNetGroupIds(itemKey),
