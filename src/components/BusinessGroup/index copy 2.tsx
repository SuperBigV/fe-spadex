...
export default function index(props: IProps) {
  ...
  const getTeamList = (search?: string, isDelete?: boolean) => {
    ...
    getBusinessTeamList(params).then((data) => {
      setBusinessTreeGroupData(listToTree(data.dat, siteInfo?.businessGroupSeparator));
      setBusiGroupsListData(data.dat || []);
    });
  };
  return (
    <Resizable
    ...
    >
      <div className={collapse ? 'spadex-biz-group-container collapse' : 'spadex-biz-group-container'}>
        ...
        >
          <Tabs.TabPane tab={'业务分组'} key='busi'>
            <div
          ...
          <Tabs.TabPane tab={'资产分组'} key='asset'>
            <div
              className='collapse-btn'
              onClick={() => {
                localStorage.setItem('leftlist', !collapse ? '1' : '0');
