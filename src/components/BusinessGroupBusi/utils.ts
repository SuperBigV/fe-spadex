...
export function getVaildAssetGroup(
  assetGroups,
  assetGroupKey: {
    key: string;
    ids: string;
  ...
  let ids = getCleanAssetGroupIds(assetGroupKey.key);
  let idsArr = _.map(_.compact(_.split(ids, ',')), _.toNumber);
  const isValid = !_.isEmpty(idsArr)
    ? _.every(idsArr, (id) => {
        if (!_.find(assetGroups, { id })) {
          return false;
        }
        return true;
      })
    : false;
  if (!isValid) {
    return getDefaultBusiness(assetGroups);
  }
  return assetGroupKey;
}
export function getCleanAssetGroupIds(assetGroupIds?: string) {
  if (!assetGroupIds) return undefined;
  return _.replace(assetGroupIds, /group,/, '');
}

export function getDefaultAssetGroupKey() {
  const { ids: defaultAssetGroupIds, isLeaf: defaultAssetGroupIsLeaf } = queryString.parse(window.location.search);
  const defaultAssetGroupKey =
    (defaultAssetGroupIds && (defaultAssetGroupIsLeaf === 'true' ? (defaultAssetGroupIds as string) : `group,${defaultAssetGroupIds}`)) ||
    window.localStorage.getItem('assetGroupKey') ||
    undefined;
  return defaultAssetGroupKey;
}
