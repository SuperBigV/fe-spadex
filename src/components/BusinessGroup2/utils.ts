...
export function getCleanAssetModelIds(businessGroupIds?: string) {
...
export function getDefaultBusinessGroupKey() {
  ...
  const defaultBusinessGroupKey =
    ...
    window.localStorage.getItem('assetModelGroupKey') ||
    undefined;
...
export function getDefaultAssetModel(busiGroups) {
  ...
  if (!isValid) {
    ...
    ids = getCleanAssetModelIds(defaultBusinessGroupKey);
    idsArr = _.map(_.compact(_.split(ids, ',')), _.toNumber);
  }
  if (defaultBusinessGroupKey) {
    window.localStorage.setItem('assetModelGroupKey', defaultBusinessGroupKey);
    return {
...
export function getVaildAssetModel(
  busiGroups,
  businessGroupKey: {
    key: string;
    ids: string;
  ...
  let ids = getCleanAssetModelIds(businessGroupKey.key);
  let idsArr = _.map(_.compact(_.split(ids, ',')), _.toNumber);
  const isValid = !_.isEmpty(idsArr)
    ? _.every(idsArr, (id) => {
        if (!_.find(busiGroups, { id })) {
    ...
    : false;
  if (!isValid) {
    return getDefaultAssetModel(busiGroups);
  }
