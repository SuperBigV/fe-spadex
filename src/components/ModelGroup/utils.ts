...
export function getCleanModelGroupIds(modelGroupIds?: string) {
  if (!modelGroupIds) return undefined;
  return _.replace(modelGroupIds, /group,/, '');
}

export function getDefaultModelGroupKey() {
  const { ids: defaultModelGroupIds, isLeaf: defaultModelGroupIsLeaf } = queryString.parse(window.location.search);
  const defaultModelGroupKey =
    (defaultModelGroupIds && (defaultModelGroupIsLeaf === 'true' ? (defaultModelGroupIds as string) : `group,${defaultModelGroupIds}`)) ||
    window.localStorage.getItem('modelGroupKey') ||
    undefined;
  return defaultModelGroupKey;
}

export function getDefaultModel(modelGroups) {
  let defaultModelGroupKey = getDefaultModelGroupKey();
  let ids = getCleanModelGroupIds(defaultModelGroupKey);
  let idsArr = _.map(_.compact(_.split(ids, ',')), _.toNumber);
  const isValid = !_.isEmpty(idsArr)
    ? _.every(idsArr, (id) => {
        if (!_.find(modelGroups, { id })) {
          window.localStorage.removeItem('modelGroupKey');
          return false;
        }
        return true;
      })
  ...
  // 缓存的节点信息无效时，取第一个节点
  if (!isValid) {
    defaultModelGroupKey = modelGroups?.[0]?.id;
    ids = getCleanModelGroupIds(defaultModelGroupKey);
    idsArr = _.map(_.compact(_.split(ids, ',')), _.toNumber);
  }
  if (defaultModelGroupKey) {
    window.localStorage.setItem('modelGroupKey', defaultModelGroupKey);
    return {
      key: _.toString(defaultModelGroupKey),
      ids,
      id: _.map(_.split(ids, ','), _.toNumber)?.[0],
      isLeaf: !_.startsWith(defaultModelGroupKey, 'group,'),
    };
  }
  return {};
}
 ...
 * @param allBusiGroups
 */
export const getModelGroupsOptions = (myBusiGroups, allBusiGroups) => {
...
export function getVaildModelGroup(
  modelGroups,
  modelGroupKey: {
    key: string;
    ids: string;
  ...
  let ids = getCleanModelGroupIds(modelGroupKey.key);
  let idsArr = _.map(_.compact(_.split(ids, ',')), _.toNumber);
  const isValid = !_.isEmpty(idsArr)
    ? _.every(idsArr, (id) => {
        if (!_.find(modelGroups, { id })) {
    ...
    : false;
  if (!isValid) {
    return getDefaultModel(modelGroups);
  }
