...
> 或者 trigger_time >= $__unixEpochFrom() AND trigger_time < $__unixEpochTo(), $__unixEpochFrom() 表示开始的unix时间戳，$\_\_unixEpochTo() 表示结束时间的 unix 时间戳
...
| `$__timeFrom()`                      | 如果时间列格式是`%Y-%m-%d %H:%i:%s`，可以使用此宏变量，将被替换为当前时间选择的开始时间。_FROM_UNIXTIME(1494410783)_                                         |
| `$__timeTo()`                        | 如果时间列格式是`%Y-%m-%d %H:%i:%s`，可以使用此宏变量，将被替换为当前时间选择的结束时间。_FROM_UNIXTIME(1494410983)_                                         |
| `$__unixEpochFilter(dateColumn)`     | 如果  `dateColumn`  格式是  Unix 时间戳。将会被替换为  *dateColumn > 1494410783 AND dateColumn < 1494497183*                                                 |
| `$__unixEpochFrom()`                 | 将被替换为当前活动时间选择的开始时间,以 Unix 时间戳表示。例如,_1494410783_                                                                                   |
| `$__unixEpochTo()`                   | 将被替换为当前活动时间选择的结束时间,  以 Unix 时间戳表示。例如,_1494497183_                                                                                 |
