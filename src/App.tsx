...
import { getBusiGroups, getDatasourceBriefList, getMenuPerm, getModelGroups, getAssetModels } from '@/services/common';
...
export interface ICommonState {
  ...
  setAssetModels: (groups: { name: string; id: number }[]) => void;
  modelGroups: {
    name: string;
    id: number;
    label_value?: string;
  ...
  setModelGroup: (group: { key?: string; ids?: string; id?: number; isLeaf?: boolean }) => void;
  assetModel: {
    key?: string; // 业务组组件本身的key
    ids?: string; // 逗号分割 'id1,id2,id3'
    id?: number; // 叶子节点的id 用于兼容旧的代码
  ...
  getVaildModelGroup: (modelGroups: any[], modelGroupKey: { key?: string; ids?: string; id?: number; isLeaf?: boolean }) => void;
  profile: IProfile;
...
function App() {
  ...
  const [commonState, setCommonState] = useState<ICommonState>({
    ...
    setModelGroups: (modelGroups) => {
      setCommonState((state) => ({ ...state, modelGroups }));
    },
    assetModels: [],
    ...
    setAssetModel: (assetModel) => {
      setCommonState((state) => ({ ...state, assetModel }));
    },
    modelGroup: {},
    setModelGroup: (modelGroup) => {
      setCommonState((state) => ({ ...state, modelGroup }));
    },
    getVaildModelGroup,
    modelGroupOnChange: (key: string) => {
      window.localStorage.setItem('modelGroupKey', key);
      const ids = getCleanModelGroupIds(key);
      setCommonState((state) => ({
        ...
        modelGroup: {
          ...
          isLeaf: !_.startsWith(key, 'group,'), // 非叶子节点
        },
      }));
    },
    ...
    getVaildAssetModel,
    assetModelOnChange: (key: string) => {
      window.localStorage.setItem('assetModelKey', key);
