...
const CreateModal: React.FC<ModalProps> = (props: ModalProps) => {
  ...
  const { visible, userType, onClose, action, userId, teamId, onSearch, width, grp_type } = props;
  ...
  const onOk = async (val?: string) => {
    ...
    if (isBusinessForm) {
      let form = teamRef.current.form;
      const { name, members, attr, label_key, label_value } = await form.validateFields();
      let params = {
        name,
        label_key,
        label_value,
  ...
  return (
    ...
    >
      ...
      {isBusinessForm && <BusinessForm ref={teamRef} businessId={teamId} action={action} grp_type={grp_type} />}
