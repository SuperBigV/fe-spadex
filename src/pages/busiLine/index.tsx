...
import { Modal, Tag, Form, Input, Alert, Select, Tooltip, Table, Tabs, TabsProps } from 'antd';
...
import List, { ITargetProps } from '@/pages/targets/List';
import MonitorScape from './MonitorScape';

import Topology from './topology';
import BusinessGroup from './BusinessGroup';
import BusinessGroup2, { getCleanBusinessGroupIds } from '@/components/BusinessGroupBusi';
import './locale';
import './index.less';
        ...
        return (
          <Form.Item label={t('update_busi.label')} name='bgids' rules={[{ required: true }]}>
            <Select
              ...
              onFocus={() => {
                getBusiGroups('', 5000, 'busi').then((res) => {
...
const Targets: React.FC = () => {
  ...
  const [tabKey, setTabKey] = useState('asset');
  useEffect(() => {
  ...
  const items: TabsProps['items'] = [
    ...
    },
    {
      key: 'monitor',
      label: '监控采集',
      children: <MonitorScape gids={gids} refreshFlag={refreshFlag} setRefreshFlag={setRefreshFlag} />,
    },
    // {
    //   key: 'alert',
    //   label: '告警',
    ...
    // },
    {
      key: 'topology',
      label: '链路',
      children: <Topology gids={gids} />,
    },
  ];

  return (
    <PageLayout icon={<DatabaseOutlined />} title={'采集配置'}>
      <div className='object-manage-page-content'>
        ...
        >
          <MonitorScape gids={gids} refreshFlag={refreshFlag} setRefreshFlag={setRefreshFlag} />
          {/* <Tabs
