...
export default function index(props: IProps) {
  ...
  const getBusiGroups = async (bgid) => {
    await getBusinessTeamInfo(bgid).then((res) => {
      const ids = res.user_groups.map((item) => _.toString(item.user_group.id));
      const currentValues = form.getFieldsValue();
      if (!form.getFieldValue('notify_groups') || form.getFieldValue('notify_groups')?.length === 0) {
        form.setFieldsValue({
          ...currentValues,
          notify_groups: ids,
        });
