...
export const typeIconMap = {
  string: (
    <span className='spadex-es-discover-fields-item-field' style={{ color: '#4a7194' }}>
      t
    </span>
  ),
  number: (
    <span className='spadex-es-discover-fields-item-field' style={{ color: '#387765' }}>
      #
    </span>
  ),
...
export default function Field(props: Props) {
  ...
  return (
    <Popover
      ...
      title={fieldLabel}
      content={
        <div className='spadex-es-discover-field-values-topn'>
          <strong>{t('log.fieldValues_topn')}</strong>
          <Spin spinning={top5Loading}>
            <div className='spadex-es-discover-field-values-topn-list'>
              {_.isEmpty(top5Data) && t('log.fieldValues_topnNoData')}
              {_.map(top5Data, (item) => {
                const percent = _.floor(item.value * 100, 2);
                return (
                  <div key={item.label} className='spadex-es-discover-field-values-topn-item'>
                    <div style={{ width: 'calc(100% - 40px)' }}>
                      <div className='spadex-es-discover-field-values-topn-item-content'>
                        <div className='spadex-es-discover-field-values-topn-item-label'>
                          {_.isEmpty(item.label) && !_.isNumber(item.label) ? '(empty)' : <Tooltip title={item.label}>{item.label}</Tooltip>}
                        </div>
                        <div className='spadex-es-discover-field-values-topn-item-percent'>{percent}%</div>
                      </div>
    ...
    >
      <div className='spadex-es-discover-fields-item' key={item}>
        <span className='spadex-es-discover-fields-item-icon'>{typeIconMap[typeMap[record.type]] || <QuestionOutlined />}</span>
        <span className='spadex-es-discover-fields-item-content'>{fieldLabel}</span>
        <span
          className='spadex-es-discover-fields-item-oper'
          onClick={() => {
