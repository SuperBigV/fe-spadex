...
import { Empty, Spin, InputNumber, Select, Radio, Space, Checkbox, Tag, Form, Alert, Input } from 'antd';
import { FormInstance } from 'antd/lib/form/Form';
import { RightOutlined, LeftOutlined } from '@ant-design/icons';
import { useLocation } from 'react-router-dom';
import { getLogsSlsQuery } from './services';
...
export default function index(props: IProps) {
  ...
  const date_field = Form.useWatch(['query', 'date_field'], form) || 'time';
  ...
  const fetchSeries = (values) => {
    if (timesRef.current) {
      const { from, to } = timesRef.current;
      metricQuery({
        ...timesRef.current,
        ...
        setDisplayTimes(`${moment.unix(from).format(TIME_FORMAT)} - ${moment.unix(to).format(TIME_FORMAT)}`);
        setSeries(res || []);
      });
    }
  };
  const fetchData = () => {
    form.validateFields().then((values) => {
      ...
      try {
        getLogsSlsQuery(values.datasourceValue, timesRef.current.from, timesRef.current.to)
          .then((res) => {
  ...
  return (
    <div className='es-discover-container'>
      ...
      <div style={{ display: 'flex', flexDirection: 'row' }}>
        <Form.Item name={['query', 'project']} style={{ marginRight: '10px' }}>
          <Space direction='vertical' size='middle'>
            <Input addonBefore='项目' defaultValue='icsoc-k8s-project-c' />
          </Space>
        </Form.Item>
        <Form.Item name={['query', 'logstore']}>
          <Space direction='vertical' size='middle'>
            <Input addonBefore='日志库' defaultValue='anta-ack' />
          </Space>
        </Form.Item>
      </div>
