const ja_JP = {
  ...
  filter: {
    title: 'フィルタ条件',
    title_tip:
      'フィルタ条件の目的は、右側のメトリクスをクリックしてメトリクスの監視データを表示する際、クエリの監視データの範囲を縮小することです。フィルタ条件 {ident="spadex01"} を設定して選択した場合、cpu_usage_idle のクエリは cpu_usage_idle{ident="spadex01"} となり、クエリ曲線の数が大幅に減少します',
    add_title: 'フィルタ条件の追加',
