...
import { createSupplier, updateSupplier } from '@/services/partner';
...
const SupplierForm: React.FC<SupplierFormProps> = ({ visible, initialValues, onCancel, onOk }) => {
  ...
  useEffect(() => {
    if (visible) {
      if (initialValues) {
        form.setFieldsValue({
          ...initialValues,
          cooperationDate: initialValues.cooperation_date ? moment(initialValues.cooperation_date) : undefined,
        });
      } else {
        form.resetFields();
      }
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isEdit && initialValues?.id) {
        await updateSupplier(initialValues.id, submitData);
  ...
  return (
    <Modal title={isEdit ? '编辑供应商' : '新建供应商'} open={visible} onOk={handleSubmit} onCancel={onCancel} width={600} destroyOnClose>
      <Form
        form={form}
        layout='vertical'
        initialValues={{
      ...
      >
        ...
        <Form.Item name='address' label='地址' rules={[{ max: 200, message: '地址不能超过200个字符' }]}>
        ...
        <Form.Item name='cooperationDate' label='合作日期' rules={[{ required: true, message: '请选择合作日期' }]}>
