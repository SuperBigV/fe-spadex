const en_US = {
  ...
  form: {
    ...
    prom: {
      ...
      read_addr_tip:
        'Usually used in the scenario of deploying the alerting engine in the edge data center. If this field is not empty, spadex-edge will use this address to access the time series database. If this field is empty, spadex-edge will use the URL above to access the time series database',
      ...
      prom_installation:
        'On the machine where spadex is deployed, run the following command to install the Prometheus time series database. For production environments, it is recommended to deploy the cluster version of VictoriaMetrics. Refer to the <a>official documentation</a> for details',
