const ja_JP = {
  ...
  form: {
    ...
    prom: {
      ...
      read_addr_tip:
        '通常は、エッジ機房でのアラートエンジンの下沉部署シナリオで使用されます。このフィールドが空でない場合、spadex-edgeはこのアドレスを使用して時系列データベースにアクセスします。このフィールドが空の場合、spadex-edgeは上記のURLを使用して時系列データベースにアクセスします',
      ...
      prom_installation:
        '平台を展開するマシン上で、以下のコマンドを実行してPrometheus時系列データベースをインストールします。生産環境では、VictoriaMetricsのクラスターバージョンを展開することをお勧めします。<a>公式ドキュメント</a>を参照してください',
